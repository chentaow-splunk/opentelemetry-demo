...
function update_otel_demo_k8s {
    ...
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-config-dashboards")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-alerting")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-nginx-metrics")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-apm-dashboard")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-demo-dashboard")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-exemplars-dashboard")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-linux-dashboard")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-opentelemetry-collector")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-postgresql-dashboard")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-dashboard-spanmetrics-dashboard")' "$SPLUNK_K8S_PATH"
    yq eval -i 'select(.kind != "ConfigMap" or .metadata.name != "grafana-datasources")' "$SPLUNK_K8S_PATH"
